...
if ($result -eq "0000" ) {
    ...
    ForEach ($line in $($usernames -split "`r`n")) { c:\puppet\arquivos\msg.exe /TIME:30 $line HOUVERAM FALHAS DE CONEXAO COM A INTERNET, VERIFIQUE SE SUA INTERNET ESTA FUNCIONAL }
    } else {
    $usernames = Get-ChildItem c:\users -directory | ForEach-Object { Write-output $_.Name }
    ForEach ($line in $($usernames -split "`r`n")) { c:\puppet\arquivos\msg.exe /TIME:30 $line $argmsg }
    }
} else {
